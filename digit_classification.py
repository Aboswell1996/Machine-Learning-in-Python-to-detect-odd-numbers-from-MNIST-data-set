...
def vectorized_activation(x):
    return np.divide(np.exp(x),(np.add(1,np.exp(x))))
...
def vectorized_derivative_activation(x):
    sig = lambda y: np.divide(np.exp(y),np.add(1, np.exp(y)))
    return np.multiply(sig(x),np.subtract(1,sig(x)))


def test_derivative(x):

    twoEY = np.multiply(2,np.exp(x))
    E2Y = np.exp(np.multiply(2,x))


    np.subtract(np.divide(np.exp(x), np.add(1, np.exp(x))),    np.divide(np.exp(np.multiply(2,x)), np.add(1,twoEY, E2Y)))
...
def compute_activations(x, W, B):
    ...
    #784 because the last column is the label (0->783 = 784 features = 28^2)

    #z_s : for i in 0 : 783,sum of Xi and Wi and add the bias for that neuron. This happens for every neuron (10 times)

    NUM_NEURONS = len(B[0])
    NUM_FEATURES = len(x) #784

    #outputNeuron_WeightedSum = 0

    ############## calculating the output of the hidden layer (10 neurons)

    for neuron in range(0, NUM_NEURONS):
        
        z_s[1][neuron] = np.dot(x, W[0][neuron])

        #sum all the features and the weights for this neuron
        #for feature in range(0, NUM_FEATURES):

        z_s[1][neuron] = np.subtract(z_s[1][neuron],B[0][neuron][0])
        #update z_s for first layer

    ####################calculating the value of the output neuron in the 3rd layer

    z_s[2]  = np.dot(W[1],vectorized_activation(z_s[1]))

    z_s[2] = np.subtract(z_s[2],B[1][0][0])

    ##################computing a_s for each layer

    a_s[1] = vectorized_activation(z_s[1])

    a_s[2] = vectorized_activation(z_s[2])
...
def backpropagation(x, t, W, B):
    ...
    #deltaOut = (t-a_s[2])*vectorized_derivative_activation(z_s[2])
    deltaOut = np.multiply( (t-a_s[2]), vectorized_derivative_activation(z_s[2]))
 
    NUM_NEURONS = len(B[0])
    NUM_FEATURES = len(x) 
    #d_b[1] = deltaOut*-1
    d_b[1] = np.multiply(deltaOut, -1)

    #calculate d_b[0] and d_w[1]
    for neuron in range(0,NUM_NEURONS):

        d_b[0][neuron] = vectorized_derivative_activation(z_s[1][neuron])*W[1][0][neuron]*deltaOut*-1
        #d_w[1][-1][neuron] = deltaOut*a_s[1][neuron]
        d_w[1][-1][neuron] = np.multiply(deltaOut, a_s[1][neuron])

    #calculate d_w[0]
    for neuron in range(0,NUM_NEURONS):

        #d_w[0][neuron] = np.multiply(np.reshape(a_s[0], (784,)), d_w[1][0][neuron])
        d_w[0][neuron] = vectorized_derivative_activation(z_s[1][neuron])*W[1][0][neuron]*deltaOut * np.transpose(a_s[0])
    # TODO: YOUR CODE ENDS HERE
